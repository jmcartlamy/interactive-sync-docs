...
const extensionComponentSchema = Joi.object({
    ...
    src: Joi.string().max(128).when('type', { is: 'image', then: Joi.required() }),
    text: Joi.string().max(256).when('type', { is: 'text', then: Joi.required() }),
    template: Joi.string()
    ...
    values: Joi.object()
        .pattern(Joi.string().alphanum().max(64), Joi.string().allow('').max(128))
        .min(1)
        .max(4)
        .when('type', [
            { is: 'radio', then: Joi.required() },
...
export default userInterfaceSchema;
